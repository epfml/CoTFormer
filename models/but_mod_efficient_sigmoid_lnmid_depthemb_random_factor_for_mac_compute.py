...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x_active, x, pos_emb_closure, cache_context, start_index, indices):
        B, T, C = x.size() # batch size, sequence length, embedding dimensionality (n_embd)
        C = self.n_embd
        # calculate query, key, values for all heads in batch and move head forward to be the batch dim
        q, k ,v  = self.c_attn(x_active).split(self.n_embd, dim=2)
        k = x
        v = x
        ...
        if self.flash:
            ...
            if attn_mask is None:
                attn_mask = self.bias[:,:,:q.shape[2],:T] == 1
...
class Block(nn.Module):
    ...
    def forward(self, x, pos_emb_closure, cache_context, start_index, indices=None, router_weights=None, active_mask=None):
        if active_mask is not None:
            
            dim1_ind = torch.nonzero(active_mask.float()).view(x.shape[0], -1, 2)[..., 1]
            dim0_ind = torch.arange(x.shape[0], device=active_mask.device, dtype=torch.long).unsqueeze(1).expand(-1, dim1_ind.shape[1])
            x[dim0_ind, dim1_ind] = x[dim0_ind, dim1_ind] + self.attn(self.ln_1(x[dim0_ind, dim1_ind] ), x, pos_emb_closure, cache_context, start_index, indices)
            x[dim0_ind, dim1_ind] = x[dim0_ind, dim1_ind] + self.mlp(self.ln_2(x[dim0_ind, dim1_ind]))
        else:
            x = x + self.attn(self.ln_1(x), x, pos_emb_closure, cache_context, start_index, indices)
...
class MoDBlock(nn.Module):
    ...
    def forward(self, x, active_mask, capacity_factor):
        T_active = active_mask[0].sum()
        assert (active_mask.sum(dim=1) == T_active).all()
        # [batch_size, sequence_length, n_embd]
        B, T, C = x.shape
        # inference time optimization: sequence length can
        # be smaller than seq len during training
        top_k = int(capacity_factor * T_active)
        ...
        min_val = router_logits.min().item()
        router_logits = router_logits.masked_fill(~active_mask.unsqueeze(-1), min_val-0.1)
    ...
    def forward(self, idx, targets=None, get_logits=False, use_cache=False, iter=None):
        ...
        for rep_idx in range(1, self.n_repeat+1):
            ...
            for block in self.transformer.h_mid:
                x = block(x, pos_emb_closure, cache_context, start_index=index_shift, active_mask=active_mask)
                x = torch.where(active_mask.unsqueeze(-1), x, x_in)
            x = self.transformer.ln_mid(x)
            if router_weights is not None:
                x = x_in * (1 - router_weights) + x * router_weights

            sum_active += active_mask.sum()
            if rep_idx < self.n_repeat:
                is_final, router_weights = self.transformer.mod[rep_idx - 1](x, active_mask, capacity_factor=mod_capacity_factors[rep_idx-1])
                active_mask = active_mask & (~is_final)


        avg_depth = torch.as_tensor(sum_active) / (B * T)  * len(self.transformer.h_mid) + len(self.transformer.h_begin) + len(self.transformer.h_end)
