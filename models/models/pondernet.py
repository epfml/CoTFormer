...
class GPTBase(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.ponder_lambda_p = config.ponder_lambda_p
        self.ponder_kl_div_loss_weight = config.ponder_kl_div_loss_weight

        # self.to_halt_logits = nn.ModuleList([
        #     nn.Sequential(
        ...
        self.to_halt_logits = nn.ModuleList([nn.Linear(config.n_embd, 1) for _ in range(self.n_repeat)])
        ...
        self.transformer.wte.weight = self.to_logits[1].weight # https://paperswithcode.com/method/weight-tying
    ...
    def forward(self, idx, targets=None, get_logits=False, use_cache=False, iter=None):
        ...
        for rep_idx in range(1, self.n_repeat+1):
            ...
            x_maybe_final = x # [B,T,D]
            for block in self.transformer.h_end:
                x_maybe_final = block(x_maybe_final, pos_emb_closure, cache_context, start_index=index_shift)
                
            halt_logits = self.to_halt_logits[rep_idx-1](x_maybe_final) # [B,T,1]
            
            hiddens.append(x_maybe_final)
            halting_logits.append(halt_logits)
            
        # stack halting probs (lambda) and y
        halting_logits = torch.stack(halting_logits, dim = 1) # [B,n_repeat,T,1]
        halting_probs = halting_logits.sigmoid()
        halting_probs_geom = calc_geometric(halting_probs, dim = 1) # [B,n_repeat,T,1]
        hiddens = torch.stack(hiddens, dim = 1) # [B,n_repeat,T,D]

        # calculating halting probs and number of active layers
        should_halt = torch.rand_like(halting_probs) <= halting_probs # [B,n_repeat,T,1]
        max_num_layers = should_halt.shape[1] # n_repeat
        halt_layer_indices = (should_halt.cumsum(dim = 1) == 0).sum(dim = 1).clamp(max = max_num_layers - 1) # [B,T]
        sum_active = (halt_layer_indices+1).sum()

        if not self.training:
            # Plot the list of frequencies for each number of exit layer e.g. [0.5, 0.25, 0.125, 0.125]
            # basically simply compute the frequencies of each number in halt_layer_indices
            # and divide by the total number of examples:
            
            freqs = torch.bincount(halt_layer_indices.view(-1), minlength=max_num_layers).float() / halt_layer_indices.numel()
            print("freqs", freqs)
            print("halt_layer_indices", list(halt_layer_indices[0].view(-1)[:20].cpu().numpy()))
            hiddens = hiddens.gather(dim=1, index=halt_layer_indices.expand(*x.shape).unsqueeze(1))
            hiddens = hiddens.squeeze(1)
            logits = self.to_logits(hiddens)
            ce_loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index = -1)
            logits = logits if get_logits else None
            return {'logits': logits, 'loss': ce_loss, 'average_depth': torch.as_tensor(sum_active / (B * T)) * len(self.transformer.h_mid) + len(self.transformer.h_begin) + len(self.transformer.h_end)}

        else:

            logits = self.to_logits(hiddens) # [B,n_repeat,T,VocabSize]
            # calculate kl div with geometric prior
            geometric_dist = calc_geometric(torch.full((self.n_repeat,), self.ponder_lambda_p, device = device)) # [n_repeat,]
            halting_probs_geom = rearrange(halting_probs_geom.squeeze(), 'b n t -> (b t) n') # [B*T,n_repeat]

            # correcting distributions so they sum to 1
            halting_probs_geom[:,-1] = 1 - halting_probs_geom[:,:-1].sum(dim=1)
            geometric_dist[-1] = 1 - geometric_dist[:-1].sum()
            halting_probs_geom = torch.clamp(halting_probs_geom, min = 0.0, max = 1.0)

            kl_div_loss = F.kl_div(
                (halting_probs_geom + 1e-8).log(),
                geometric_dist.expand_as(halting_probs_geom),
                None, None,
                'batchmean'
            )

            # calculate cross entropy loss
            targets = repeat(targets, 'b t -> (b n t)', n = self.n_repeat) # [B*n_repeat*T,]
            logits = rearrange(logits, 'b n t v -> (b n t) v') # [B*n_repeat*T,VocabSize]
            ce_loss = F.cross_entropy(logits, targets, ignore_index = -1, reduction='none')

            weighted_ce_loss = ce_loss * halting_probs_geom.view(-1)
            #print(f"[train] ce_loss: {ce_loss.mean().item():.4f}, kl_div_loss: {kl_div_loss.mean().item():.4f}, weighted_ce_loss: {weighted_ce_loss.mean().item():.4f}, active: {len(self.transformer.h_mid)} {sum_active} {torch.as_tensor(sum_active / (B * T)) * len(self.transformer.h_mid)}")

            # sum loss
            loss = weighted_ce_loss.mean() + self.ponder_kl_div_loss_weight * kl_div_loss.mean()
            
            if use_cache:
                x = self.lm_cache.get_final_logits(x_maybe_final)
    ...
    def get_parameter_group_specs(self):
        ...
        decay.remove('to_logits.1.weight')
